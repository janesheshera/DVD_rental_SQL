...
row_number() over(order by payment_date), --Пронумеровать все платежи от 1 до N по дате.
row_number() over(partition by customer_id order by payment_date), --Пронумеровать платежи для каждого покупателя, сортировка платежей должна быть по дате.
sum(amount) over(partition by customer_id order by payment_date), --Расчет нарастающим итогом суммы всех платежей для каждого покупателя, сортировка должна сперва по дате платежа, а затем по сумме платежа от наименьшей к большей.
dense_rank() over(partition by customer_id order by amount) --Нумерация платежей для каждого покупателя по стоимости платежа от наибольших к меньшим так, чтобы платежи с одинаковым значением имели одинаковое значение номера.
from payment
...
select payment_id, customer_id, payment_date, amount, lag(amount, 1, 0.) over(partition by customer_id order by payment_id)
from payment
...
select *
from (select *, row_number() over (partition by customer_id order by payment_date desc) from payment) t
where row_number = 1

-- 5. С помощью оконной функции вывести для каждого сотрудника сумму продаж за апрель 2007 года:
select *, sum(amount) over(partition by staff_id) from payment p 
...
select *, r from (select *, row_number() over (order by payment_id) % 100 = 0 as r from payment) as fo
